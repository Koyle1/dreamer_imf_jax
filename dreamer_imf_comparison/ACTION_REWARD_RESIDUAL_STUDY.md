...
The old training bank contains five-step candidate suffixes and labels only at horizons 1, 3, and 5. The dense study therefore does not interpolate missing labels. It keeps the exact same simulator states and five-step causal intervention, appends the recorded behavior-policy actions as a common candidate-independent suffix through step 15, and replays the simulator to obtain all 15 cumulative returns. The regenerated legacy labels must agree within $10^{-6}$. The original bank and both earlier result trees remain immutable. The dense study uses a new schema and output root and records matched deltas against both earlier residual versions.
