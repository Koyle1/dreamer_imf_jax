...
class SequenceReplayBuffer:
    ...
    def sample(
        ...
        *,
        ...
        """Sample burn-in context followed by loss-bearing sequence steps.

        ``sequence_length`` always denotes the number of optimized steps.  The
        returned time axis has length ``burn_in + sequence_length`` and carries
        both the true episode-start flags and an explicit loss mask.  Existing
        callers that omit ``burn_in`` retain the previous shape and receive an
        all-one mask.
        """
        ...
        materialized_length = burn_in + sequence_length
        items = list(self._items)
        valid = self._valid_starts(materialized_length)
        if not valid:
        ...
        rows = [
            [items[int(start) + offset] for offset in range(materialized_length)]
            for start in starts
        ]
        loss_mask = np.ones((batch_size, materialized_length), dtype=np.float32)
        ...
        arrays = {
            ...
            "is_first": jnp.asarray(
                [[item.is_first for item in row] for row in rows], dtype=jnp.bool_
            ),
